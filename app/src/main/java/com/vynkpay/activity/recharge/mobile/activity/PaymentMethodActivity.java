...
public class    PaymentMethodActivity extends AppCompatActivity implements PaymentResultWithDataListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        ButterKnife.bind(PaymentMethodActivity.this);
        sp = getSharedPreferences("PREFS_APP_CHECK", Context.MODE_PRIVATE);

        MainApplication.getApiService().getProfileMethod(Prefes.getAccessToken(PaymentMethodActivity.this)).enqueue(new Callback<GetProfileResponse>() {
            @Override
            public void onResponse(Call<GetProfileResponse> call, retrofit2.Response<GetProfileResponse> response) {
                ...
                if(response.isSuccessful() ){
                    if(response.body().getSuccess()){
                        ...
                        Prefes.clear(PaymentMethodActivity.this);
                        ...
                        if(sp.getString("value","").equals("Global") && Prefes.getisIndian(getApplicationContext()).equals("NO")){
                            ...
                            startActivity(new Intent(PaymentMethodActivity.this, HomeActivity.class).putExtra("Country", "India").addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP));
                            finish();
                        }
        ...
        dialog = M.showDialog(PaymentMethodActivity.this, "", true, true);
        ...
        Log.e("operatorDetailid1",""+Prefes.getAccessToken(PaymentMethodActivity.this));
    ...
    private void makeDialogSuccess(String txnId, String dateTime, String amount, String _MOBILE_NUMBER, String title, String mSuccess) {
        successDialog = M.inflateDialog(PaymentMethodActivity.this, R.layout.dialog_payment_result_rcg);
        ...
        if (_TYPE.equals("5")){
            forWhatPaid.setText("DTH Recharge");
    ...
    private void makeErrorDialog(String mMessage, final String errorCode) {
        errorDialog = M.inflateDialog(PaymentMethodActivity.this, R.layout.dialog_error_message_rcg);
    ...
    private void fetchWalletData() {
        Dialog loader=M.showDialog(PaymentMethodActivity.this, "", false, false);
        loader.show();
        MainApplication.getApiService().walletAmountMethod(Prefes.getAccessToken(PaymentMethodActivity.this)).enqueue(new Callback<GetWalletResponse>() {
            @Override
            public void onResponse(Call<GetWalletResponse> call, Response<GetWalletResponse> response) {
                if(response.body().getSuccess()){
                    ...
                    MainApplication.getApiService().redeemAmountMethod
                            (Prefes.getAccessToken(PaymentMethodActivity.this),_OPERATOR_ID,_AMOUNT).enqueue(new Callback<ReddemAmountResponse>() {
                        @Override
    ...
    private void setListeners(String totalAmount) {
        ...
        if (!payUCheck.isChecked() && !orendaCheck.isChecked()){
            makepaymentButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Toast.makeText(PaymentMethodActivity.this, "Please select one of the option to pay.", Toast.LENGTH_SHORT).show();
                }
            });
        }
    ...
    public void onPaymentSuccess(String s, PaymentData paymentData) {
        ...
        try {
            ...
            MainApplication.getApiService().addMoneyRazorMethod(Prefes.getAccessToken(PaymentMethodActivity.this),
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, BuildConfig.APP_BASE_URL + URLS.recharge,
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                Log.i(">>params", "getParams: " +
                        _MOBILE_NUMBER + "\n" +
                        AMOUNT_FOR_ORENDA + "\n" +
                        _OPERATOR_ID + "\n" + _TYPE + "\n" + Prefes.getDescription(PaymentMethodActivity.this) + "\n" +
                        Prefes.getTalktime(PaymentMethodActivity.this) + "\n" +
                        Prefes.getValidity(PaymentMethodActivity.this)
                        + "\n" + Prefes.getPlan(PaymentMethodActivity.this));


                params.put("type", Prefes.getType(PaymentMethodActivity.this));
                params.put("operator_url_type", Prefes.getOperatorUrl(PaymentMethodActivity.this));
                params.put("mobile_number", Prefes.getPhoneNumber(PaymentMethodActivity.this));
                params.put("operator_id",Prefes.getOperatorDID(PaymentMethodActivity.this));
                params.put("circle", Prefes.getCircle(PaymentMethodActivity.this));
            ...
            public Map<String, String> getHeaders() throws AuthFailureError {
                Map<String, String> params = new HashMap<String, String>();
                params.put(ApiParams.access_token, Prefes.getAccessToken(PaymentMethodActivity.this));
